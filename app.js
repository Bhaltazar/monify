...
export { db, fmt, fmtDate, today, showToast, showConfirm, openModal, closeModal, currentUser, currentQuincenaId, quincenas, currentTab, userConfig };
...
window.createQuincena = async () => {
  ...
  try {
    ...
    startMovsListener(); startMotoListener();
...
// ── MAIN RENDER ───────────────────────────────────────
const BALANCE_LABELS_DEFAULT = ['Inicio','Disponible','Gastado','Ahorrado'];
const BALANCE_IDS_DEFAULT=['saldo-inicial-display','saldo-actual-display','total-gastado-display','total-ahorrado-display'];
const BALANCE_CLASSES_DEFAULT=['v-accent','v-green','v-red','v-teal'];
function render(){
  document.getElementById('fab-btn').textContent=currentTab==='prestamos'?'＋ Nuevo préstamo':currentTab==='kms'?'＋ Registrar carga':'＋ Agregar movimiento';
  const gasChip=document.getElementById('gas-price-chip');
  if(gasChip){
    gasChip.style.display=currentTab==='kms'?'flex':'none';
    if(currentTab!=='kms') gasChip.classList.remove('expanded');
  }
  ...
  if(currentTab==='kms'){
    ...
    BALANCE_LABELS_DEFAULT.forEach((label,i)=>{
      const el=document.getElementById('bal-label-'+(i+1));
      if(el) el.textContent=label;
    });
    BALANCE_IDS_DEFAULT.forEach((id,i)=>{
      const el=document.getElementById(id);
      if(el) el.className='balance-item-value '+BALANCE_CLASSES_DEFAULT[i];
    });
    updateHeader();
  }
  rebuildNavTabs();
...
document.getElementById('fab-btn').addEventListener('click',async()=>{
  if(currentTab==='kms'){window.openGasModal();return;}
...
async function checkSectionDisable(section,isChecked){
  ...
  if(section==='kms'&&currentUser){
    try {
      const snap=await getDocs(query(collection(db,'motoCargas'),where('uid','==',currentUser.uid),limit(1)));
