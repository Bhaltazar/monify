...
let gasChipTimer = null;

window.toggleGasPriceChip = () => {
  const chip = document.getElementById('gas-price-chip');
  if(!chip) return;
  if(chip.classList.contains('expanded')){
    if(gasChipTimer){ clearTimeout(gasChipTimer); gasChipTimer=null; }
    window.openGasPriceModal();
  } else {
    chip.classList.add('expanded');
    if(gasChipTimer) clearTimeout(gasChipTimer);
    gasChipTimer = setTimeout(()=>{ collapseGasPriceChip(); }, 3000);
  }
};
